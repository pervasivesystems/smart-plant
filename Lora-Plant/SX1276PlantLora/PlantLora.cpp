...
AnalogIn light_value(A0);
AnalogIn temp_value(A2);
AnalogIn soil_value(PA_5);
DigitalOut motor(D7);
...
void setSensorMessage(){
    ...
    float light;
    light = light_value.read() *100 + randomBit; // Read the analog input value (value from 0.0 to 1.0 = full ADC conversion range)
    dprintf("Light = %f",light);
    
    randomBit = rand() % 2;
    float temp;
    temp = (temp_value.read() * 4000) - 10.0 - randomBit;
    
    //temp =  * (50000/1024); // Read the analog input value (value from 0.0 to 1.0 = full ADC conversion range)
    dprintf("Temp = %f",temp);
    
    randomBit = rand() % 2;
    float soil;
    soil = soil_value.read() *10; // Read the analog input value (value from 0.0 to 1.0 = full ADC conversion range)
    dprintf("Soil = %f",soil);
    soil = 10.3253 - randomBit;
    
    char message[32];//Message to be sent
    sprintf(message, "#%f,%f,%f#@@", light, temp, soil);
    dprintf(message);
